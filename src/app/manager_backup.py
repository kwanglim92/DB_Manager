...
class DBManager:
    ...
    def perform_qc_check(self):
        """QC 검수 실행"""
        if not hasattr(self, 'qc_type_var') or not hasattr(self, 'equipment_types_for_qc'):
            messagebox.showerror("오류", "QC 시스템이 초기화되지 않았습니다.")
            return
        ...
        try:
            ...
            # 트리뷰 초기화
            if hasattr(self, 'qc_result_tree'):
            ...
            SELECT 
                ...
                d.max_spec, 
                d.default_value,
                COALESCE(d.confidence_score, 1.0) as confidence_score,
                COALESCE(d.occurrence_count, 1) as occurrence_count,
                COALESCE(d.total_files, 1) as total_files,
                d.created_at
            ...
            if not data:
                loading_dialog.close()
                messagebox.showinfo("알림", f"장비 유형 '{selected_type}'에 대한 검수할 데이터가 없습니다.")
                conn.close()
                return
            ...
            # 기본적인 QC 체크들
            for row in data:
                param_name, min_spec, max_spec, default_value = row[0], row[1], row[2], row[3]
                ...
                # 2. 스펙 범위 체크
                if min_spec and max_spec and default_value:
                    try:
                        ...
                        if default_val < min_val or default_val > max_val:
                            results.append({
                                "parameter": param_name,
                                "issue_type": "범위 초과",
                                "description": f"기본값 {default_val}이 허용 범위 [{min_val}, {max_val}]를 벗어났습니다",
                                "severity": "높음"
                            })
                    except ValueError:
                        results.append({
                            "parameter": param_name,
                            "issue_type": "데이터 타입",
                            "description": "숫자가 아닌 값이 포함되어 있습니다",
                            "severity": "중간"
                        })
    ...
    def show_qc_statistics(self, results):
        ...
        if not results:
            ttk.Label(self.stats_frame, text="✅ 이슈가 발견되지 않았습니다.", 
                     font=("Arial", 12, "bold"), foreground="green").pack(padx=10, pady=10)
            return
        ...
        # 통계 표시
        ttk.Label(self.stats_frame, text=f"총 이슈 수: {len(results)}건", 
                 font=("Arial", 12, "bold")).pack(anchor="w", padx=10, pady=5)

        # 심각도별 통계
        ttk.Label(self.stats_frame, text="심각도별 통계:", 
                 font=("Arial", 10, "bold")).pack(anchor="w", padx=10, pady=2)
        for severity, count in severity_counts.items():
            if count > 0:
                color = {"높음": "red", "중간": "orange", "낮음": "blue"}.get(severity, "black")
                ttk.Label(self.stats_frame, text=f"• {severity}: {count}건", 
                         foreground=color).pack(anchor="w", padx=20, pady=1)

        # 이슈 유형별 통계
        ttk.Label(self.stats_frame, text="이슈 유형별 통계:", 
                 font=("Arial", 10, "bold")).pack(anchor="w", padx=10, pady=2)
