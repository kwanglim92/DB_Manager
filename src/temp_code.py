"""
이 파일은 DB_Manager.py의 add_to_default_db 함수 중복 정의 문제를 해결하기 위한 파일입니다.
DB_Manager.py에서 모든 add_to_default_db 함수를 제거하고 이 함수로 대체하세요.
"""
...
def add_to_default_db(self):
    ...
    def on_confirm():
        ...
        for param_name, p_values in items_to_potentially_add:
            cursor.execute(
                "SELECT id FROM Default_Values WHERE equipment_type_id = ? AND parameter_name = ?",
